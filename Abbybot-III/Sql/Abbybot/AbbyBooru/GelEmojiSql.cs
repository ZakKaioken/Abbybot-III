...
namespace Abbybot_III.Sql.Abbybot.AbbyBooru
{
	class GelEmojiSql
	{
		...
		{
			AbbyTable table = await AbbySql.AbbysqlClient.FetchSQL($"SELECT GelbooruCommandJson FROM abbybooru.emojis WHERE `MessageId`='{id}' AND `Emoji`='{name}' AND `OwnerId`='{userId}';");
		...
		public static async Task AddEmojiAsync(ulong oid, ulong mid, string t, int type, string msgjson, string resjson)
		{
			...
			await AbbySql.AbbysqlClient.RunSQL($"insert into abbybooru.emojis (`OwnerId`, `MessageId`, `Emoji`, `Type`, `GelbooruCommandJson`, `GelbooruResultJson`) values ('{oid}', '{mid}', '{emoji}', '{type}', '{jsoni}', '{jsoni2}');");
		}
		...
		{
			AbbyTable table = await AbbySql.AbbysqlClient.FetchSQL($"SELECT GelbooruResultJson FROM abbybooru.emojis WHERE `MessageId`='{id}' AND `Emoji`='{name}' AND `OwnerId`='{userId}';");

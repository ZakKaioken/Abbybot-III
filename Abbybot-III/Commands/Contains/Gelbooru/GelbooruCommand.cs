...
namespace Abbybot_III.Commands.Contains.Gelbooru
{
	class GelbooruCommand : ContainCommand
	{
		...
		async Task PostImages(AbbybotCommandArgs msg, ImgData im)
		{
			...
			{
				var fc = im.user.FavoriteCharacter;
		...
		{
			bool loli = msg.guild.NoLoli && im.loli;
			bool shot = msg.guild.NoLoli && im.shot;
			bool nsfw = msg.guild.NoNSFW && im.nsfw;
		...
		async Task GetImage(ImgData im)
		{
			try
			{
				var badtaglisttags = await UserBadTagListSql.GetbadtaglistTags(im.user.Id);
		...
		async Task<bool> CanRun(AbbybotCommandArgs md)
		{
			...
			{
				if (!md.user.Ratings.Contains((CommandRatings)2))
				{
					safe = true;
				}
				if (!md.user.Ratings.Contains((CommandRatings)3))
				{
					taggs.Add("-loli");
				}
			}
			if (!isnsfwchannel || !md.user.IsLewd || safe)
			{
			...
			if (md.sudoUser == null)
			{
				...
				{
					sb.Append((md.user.FavoriteCharacter.Length > 1) ? md.user.FavoriteCharacter : "abigail_williams");
				}
			...
			if (md.channel is ITextChannel sgc)
			{
				...
				{
					if (md.user.Ratings.Contains(Rating))
					{
						canrun = true;
					}
				}
